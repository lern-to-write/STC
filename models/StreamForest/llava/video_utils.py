...
def get_frame_indices(num_frames, vlen, sample='middle', fix_start=None, input_fps=1, min_num_frames=1, max_num_frames=-1, local_num_frames=8):
    ...
    if sample == 'dynamic_fps1':
        ...
        num_segments = int( (duration+1) // local_num_frames)
        if num_segments == 0:
...
def read_frames_gif(
    ...
    return frames, real_frame_indices, float(fps), duration # for tgif
...
def read_frames_img(
        video_path, num_frames, sample='rand', fix_start=None, min_num_frames=1,
        max_num_frames=-1, client=None, clip=None, local_num_frames=8, fps=None
    ):
    
    def filter_image_files(frame_paths):
        # Keep only files with specific image extensions
        valid_extensions = {'.jpg', '.jpeg', '.png'}
        return [path for path in frame_paths if os.path.splitext(path)[1].lower() in valid_extensions]
    ...
    def sort_frames(frame_paths):
        frame_paths = filter_image_files(frame_paths)
    ...
    if fps is None:
        if 'tvqa' in video_path.lower():
            fps = 3.0 
        elif 'got-10k' in video_path.lower():
            fps = 10.0
        elif 'lasot' in video_path.lower():
            fps= 30.0
        else:
    ...
    if min_num_frames > vlen:
        if sample == 'dynamic_fps1':
            min_num_frames = math.ceil(vlen / local_num_frames) * local_num_frames
        else:
            min_num_frames = vlen

    if sample == 'dynamic_fps1':
        num_segments = math.ceil((duration + 1)/local_num_frames)
        if num_segments == 0:
            num_frames = local_num_frames
        else:
            num_frames = local_num_frames * num_segments
    ...
    if clip is not None:
        def _get_index_by_time(start_sec, end_sec, num_segments=8, fps=1.0, max_frame=9999):
    ...
    for idx in frame_indices:
        ...
        imgs.append(img)
        # print("idx", idx, "frame_fname:", frame_fname)
...
if __name__ =="__main__":
    ...
    # frames, frame_indices, fps, duration = read_frames_gif(
    #     video_path, num_frames=1, sample='dynamic_fps1', fix_start=None, 
    #     min_num_frames=1, max_num_frames=256, client=client, clip=None, local_num_frames=1
    # )
    # sec = [str(round(f / fps, 1)) for f in frame_indices]
    # print("frames len: ", len(frames))
    # print("frame_indices len: ", len(frame_indices))
    # print("fps: ", fps)
    # print("duration: ", duration)
    # print("sec: ", sec)
    
    
    # video_path = "p2:s3://star/Charades_v1_480/B7HDN.mp4"
    # clip=[0.0, 16.5]
    # frames, frame_indices, fps, duration = read_frames_decord(
    #     video_path, num_frames=1, sample='dynamic_fps1', fix_start=None, 
    #     min_num_frames=1, max_num_frames=256, client=client, clip=clip, local_num_frames=1
    # )
    # sec = [str(round(f / fps, 1)) for f in frame_indices]
    # print("frames len: ", len(frames))
    # print("frame_indices len: ", len(frame_indices))
    ...
    # video_path = "pnorm2:s3://lasot/kangaroo/kangaroo/kangaroo-12/img"
    # video_path = "/mnt/petrelfs/share/videointern/GOT/OpenDataLab___GOT-10k/raw/GOT-10k/train/GOT-10k_Train_006455"
    # clip=[1.0, 11.0]
    # frames, frame_indices, fps, duration = read_frames_img(
    #     video_path, num_frames=1, sample='dynamic_fps1', fix_start=None, 
    #     min_num_frames=1, max_num_frames=256, client=client, clip=clip, local_num_frames=1
    # )
    # sec = [str(round(f / fps, 1)) for f in frame_indices]
    ...
    # video_path = "/mnt/petrelfs/share_data/zengxiangyu/Tmp/AVA/frames_1fps/Lg1jOu8cUBM"
    # # video_path = "/mnt/petrelfs/share/videointern/GOT/OpenDataLab___GOT-10k/raw/GOT-10k/train/GOT-10k_Train_006455"
    # clip=[430, 490]
    # frames, frame_indices, fps, duration = read_frames_img(
    #     video_path, num_frames=1, sample='dynamic_fps1', fix_start=None, 
