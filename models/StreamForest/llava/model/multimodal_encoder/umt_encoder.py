...
def build_vit(config, pt_type='origin'):
    model = PretrainVisionTransformer(
        ...
        ckpt_num_frame=1,   # <<<Mark>>> NOTE: Only 1 for online training.   And 4 for stage1-3. <<<Mark>>>
        with_ln=True, # merge vision_layernorm in it
    )
    
    if pt_type == 'stage1_qformer':
        ckpt_path = "/mnt/petrelfs/share_data/likunchang/model/videochat2/umt_l16_qformer.pth"
        state_dict = torch.load(ckpt_path, map_location='cpu')
    elif pt_type == 'stage2_qformer_mistral':
        ckpt_path = "/mnt/petrelfs/share_data/likunchang/model/videochat2/videochat2_mistral_7b_stage2.pth"
        state_dict = torch.load(ckpt_path, map_location='cpu')
    else:
        ckpt_path = "/mnt/petrelfs/share_data/zengxiangyu/models/VideoChat-Flash/Video_Encoders_for_Training_VideoChat-Flash/UMT-L_f4_vision.pt"
