...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_image_video_new(self, images_list, video_idx_in_batch):
        ...
        if has_image:
            ...
            images_features = self.get_model().mm_projector(concat_images, local_num_frames=1)
        ...
        if has_video:
            ...
            videos_features = [v.reshape(1, -1, v.shape[-1]) for v in torch.split(videos_features, video_split_sizes)]
        ...
        if has_image and has_video:
            ...
            assert (len(videos_features) + len(images_features)) == bs
            for idx in range(bs):
                if idx in video_idx_in_batch:
                    all_videos_or_images_features.append(videos_features[vid_idx])
                    vid_idx += 1
                else:
                    all_videos_or_images_features.append(images_features[img_idx])
                    img_idx += 1
    ...
    def encode_image_video_memory(self, images_list, video_idx_in_batch):
        ...
        if has_image:
            ...
            # print("images_features:", images_features.shape)
            # images_features = torch.split(images_features, image_split_sizes)
        ...
        if has_video:
            ...
            concat_videos = torch.cat([video.reshape(video.shape[0] // mm_local_num_frames, -1, video.shape[-1]) for video in concat_videos], dim=0)
            videos_features = self.get_model().mm_projector(concat_videos, local_num_frames=mm_local_num_frames)
            # print("videos_features: ", videos_features.shape)
            # videos_features = [v.reshape(1, -1, v.shape[-1]) for v in torch.split(videos_features, video_split_sizes)]
    ...
    def encode_image_video_memory_batch(self, images_list, video_idx_in_batch):
        assert len(video_idx_in_batch)<2, "<<<encode_image_video_memory>>> i dont want to process multiple image or video"
        # input: list output: list image和video没法concat呀
        # print("images_list shape: ", images_list[0].shape)
        batch_size = 128
    ...
    def encode_video_image_memory(self, images_list, video_idx_in_batch):
        ...
        if has_image:
            image_split_sizes = [image.shape[0] for image in concat_images] 
            concat_images = torch.cat([image.unsqueeze(1) for image in concat_images], dim=0)
            # print("input vit image.shape:", concat_images.shape)   #[7, 1, 3, 224, 224]
            images_features = self.get_model().get_vision_tower()(concat_images) # B_i, N, D
            images_features = torch.split(images_features, image_split_sizes)
        ...
        if has_video:
            video_split_sizes = [video.shape[0] // mm_local_num_frames for video in concat_videos]
            concat_videos = torch.cat([video.reshape(video.shape[0] // mm_local_num_frames, mm_local_num_frames, video.shape[1], video.shape[2], video.shape[3]) for video in concat_videos], dim=0)
            # print("input vit video.shape:", concat_videos.shape)    #[32, 4, 3, 224, 224]
            videos_features = self.get_model().get_vision_tower()(concat_videos) # B_v, N, D
            videos_features = [v.reshape(-1, v.shape[-2] // mm_local_num_frames, v.shape[-1]) for v in torch.split(videos_features, video_split_sizes)]
        ...
        for idx in range(bs):
            ...
            if idx in video_idx_in_batch:
                feat = self.get_model().mm_projector(videos_features[vid_idx], is_image=False, local_num_frames=getattr(self.config, "mm_local_num_frames", -1))
                vid_idx += 1
                # print("video after projector: ", feat.shape)
            else:
                feat = self.get_model().mm_projector(images_features[img_idx], is_image=True)
                img_idx += 1
                # print("image after projector: ", feat.shape)
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            elif vision_encode_type == "image_video_memory":
            ...
            elif vision_encode_type == "video_image_memory": #UMT当单帧时，video_idx_in_batch为[]；当多帧时，video_idx_in_batch是[0]
                image_features = self.encode_video_image_memory(images_list, video_idx_in_batch=video_idx_in_batch)
            ...
            rank0_print("<<NOTE>> Maybe i'dont want to process img here...")
        ...
        if mm_llm_compress:
            raise "I dont want to use llm compress"
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0: 
                ...
                cur_image_idx += 1
                rank0_print("<<Warning>> num_images == 0, maybe there are some bug in training data?", "len(image_features): ",len(image_features), "image_features[0].shape: ", image_features[0].shape, "input_ids:", input_ids)
                continue
            if num_images>1:
                rank0_print("<<Warning>> num_images > 1, maybe there are some bug in training data?", "len(image_features): ",len(image_features), "image_features[0].shape: ", image_features[0].shape, "input_ids:", input_ids)
    ...
    def prepare_inputs_labels_for_LLM(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, image_features, modalities=["image"], image_sizes=None):
        # Note position_ids is none here
        assert type(modalities) is list, modalities
